...
Hopfield Networks can be viewed as content-addressed memory - a way to remember data and then look it up again given similar data.
...
In the traditional formulation, a Hopfield Network is a collection of weights connection nodes.
...
Now that we have a function that converges to some value to, the only thing left to do is to make sure that it converges to something useful!
The entire process is very similar to gradient descent on $$\vec{\xi}$$ using the $$E(\vec{\xi})$$, except that we are doing the entire process in binary and we are changing the values of the nodes instead of the weights.
...
So, now we have established a process that converges to something, we just have to rig it so that converges to something **useful**.

## Details

In the classic version, the energy function is given by :

$$E(\vec{\xi}) = -0.5 \sum_i x_i (\sum_{j \neq i} w_{ij} \xi_j + b_i)$$

If you stare at it hard enough, you can convince yourself that making $$x_i = -1$$ whenever $$\sum_{j \neq i} w_{ij} \xi_j + b_i$$ is negative will result in a lower energy than making $$x_i =  +1$$, assuming you hold everything else static.
We can formalize this as our update rule in vector notation: 

$$\xi_i^{t+1} \leftarrow \theta(\vec{w_{i}} \xi^{t} + b_i)$$

where $$\theta$$ is the sign function, $$w_{i}$$ is the $$i^{th}$$ row of $$W$$ and $$\xi$$ is represented as a column vector. (Note: here, we assume that $$W_{i,i}=0$$).
...
When this update rule reaches a fixed point, we have converged. Notably, when $$W := (\xi^t)^T(\xi^t)$$ and $$b := \vec{0}$$, we've reached a fixed point.
More generally, if we have a set of vectors $$X = \{\xi_0, \xi_1, ..., \xi_n\}$$ that we want to remember, setting $$W := X^TX$$ and $$b := \vec{0}$$ will result in a fixed point at each of the vectors $$\xi_{0..n}$$.
