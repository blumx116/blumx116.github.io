...
Following this logic through to the end, and you can train the second network on the previous observations of the agent.
Then, when the agent visits state $$s$$, you can give the agent bonus equal to the second network's error on $$s$$, because the error will be higher the more 'novel' $$s$$ is.
