...
***ArXiV***: *https://arxiv.org/pdf/2008.02217.pdf*
 
## Background

If you haven't read the previous post in this sequence or are unfamiliar with Hopfield networks, I recommend you read that first [here](https://blumx116.github.io/blog/review/2019/08/06/hopfield1.html).
...
for a query column vector $$\vec{\xi}$$ and $$W := X^TX$$, where the rows of $$X = \{x_0, x_1, ..., x_n\}$$ correspond to the memories/patterns stored in the network.
This corresponds to the following update rule :

$$\vec{\xi}^{t+1} = f(\vec{\xi}^t, X) = \theta(W\vec{xi}^t + \vec{b}) = \theta(X^TX\vec{\xi}^t)$$

Fast forward to modern day, a more common energy function is $$E(\vec{\xi})$$ is $$E(\vec{\xi})) = -\sum_i \exp (\vec{\xi}x_i^T)$$.
They can now store $$2^{n/2}$$ patterns of length $$n$$, but they still only operate on 1's and 0's.
...
The authors sum it up pretty well in a single graphic : 
...
I'll assume working knowledge of transformers - if you aren't famimliar with them, there are plenty of tutorials on the internet.
...
Instead, I'll try to repeat key takeaways from this section of the paper.
 ...
 - *This process always converges* : If you treat these layers as hopfield layers, you'll always converge to some value $$\vec{\xi}^*}$$ regardless of the $$\vec{\xi}$$ you put in $$^*$$
...
(It's evaluated on a language task, you can think of perplexity as an error measure.) 
...
The authors also have a companion paper evaluating the Hopfield layers on some biological datasets.
